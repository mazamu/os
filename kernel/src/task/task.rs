...
use crate::config::{MAX_SYSCALL_NUM, BIG_STRIDE, LOG};
...
trace!("{}{}", module_path!(), "::exec");
        // memory_set with elf program headers/trampoline/trap context/user stack
        let (memory_set, mut user_sp, entry_point) = MemorySet::from_elf(elf_data);//虚拟地址空间、用户栈顶、应用程序入口
        // info!("{}{}{}", module_path!(), "::exec", "--->create the virtual address memory of app");
        let trap_cx_ppn = memory_set
            .translate(VirtAddr::from(TRAP_CONTEXT).into())
            .unwrap()
            .ppn();
